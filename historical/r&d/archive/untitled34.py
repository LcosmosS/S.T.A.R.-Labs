...
def random_fibonacci_pair(n, classifier=None, fib_list=None, X_data=None, force_failure=False, seen_pairs=None):
    ...
    high_rank_pairs = [(2, 144), (3, 144), (5, 144), (2, 233), (8, 610), (5, 377), (34, 610), (2, 377), (5, 233), (13, 144), (21, 144), (34, 377), (8, 144), (55, 144), (13, 377), (34, 233), (89, 144), (5, 610)]
    high_rank_fibs = [2, 144, 3, 5, 233, 8, 610, 377, 34, 13, 21, 55, 89]
    seen_pairs = seen_pairs or {}

    if classifier is None or X_data is None or len(X_data) < 10:
        if random.random() < 0.999 and high_rank_pairs:  # 99.9% bias
...
def analyze_curve(a, b, is_original=False, max_attempts=5, require_3selmer=False):
    ...
    if conductor > 5e9 and not is_original:
    ...
    if success:
        try:
            ...
            comoving_volume = (omega * reg * cosmo_scale**3) / (1.5e13 if rank == 3 else 1e14 if rank == 2 else 7e13 if rank == 1 else 1e14)
            scaled_reg = reg * SQRT_KAPPA * (20 if rank == 3 else 10 if rank == 2 else 7 if rank == 1 else 20)
            ...
            print(f"Scaled regulator (Reg * √κ * {'20' if rank == 3 else '10' if rank == 2 else '7' if rank == 1 else '20'}): {float(scaled_reg)} (density height)")
            print(f"Product of Tamagawa numbers: {tamagawa} (local edge constraints)")
            print(f"Estimated comoving volume (Omega * Reg * scale^3 / {'1.5e13' if rank == 3 else '1e14' if rank == 2 else '7e13' if rank == 1 else '1e14'}): {comoving_volume} Mly^3")
    ...
    if success and rank is not None:
        try:
            ...
            density_size = min(leading_coeff * 177 / 30, 120) if leading_coeff else 10  # Larger rank 3
            ...
            if rank > 0:
                contour_y = np.full_like(x_vals, float(5 if rank == 3 else min(scaled_reg / 600, 5)))
...
while successful_curves < max_successful_curves and attempts < max_total_attempts:
    ...
    if attempts % 10 == 0 and len(X_data) >= 10 and len(set(y_data)) >= 2:
        ...
        classifier = LogisticRegression(max_iter=1000, class_weight={1: 30, 0: 1})  # Higher rank 3 weight
        ...
        if X_data:
            ...
            for feat in rank3_features:
                synth_feat = [f + random.gauss(0, 0.2) for f in feat]  # Larger variance
