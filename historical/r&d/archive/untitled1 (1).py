...
warnings.filterwarnings("ignore", category=FutureWarning)  # Suppress scikit-learn deprecation warnings

# Increase PARI stack size to 2GB (reduced for MyBinder stability)
pari = Pari()
...
def analyze_curve(a, b, max_bound=30):
    ...
    try:
        ...
        try:
            ...
            try:
                E.two_descent(second_limit=8, verbose=False)
                gens = E.gens()
                features['algebraic_rank'] = len(gens)
            except:
                points = E.points(bound=150)
...
def collect_initial_data(n, num_pairs=15):
...
def train_symbolic_regressor(data, n_iter_search=8):
    ...
    # Narrowed hyperparameter search space to reduce failures
    param_dist = {
        'population_size': [500, 1000],
        'generations': [10, 15],
        'parsimony_coefficient': [0.01, 0.05],
        'p_crossover': [0.7, 0.8],
    ...
    try:
        ...
        print(f"Best symbolic expression: {search.best_estimator_._program}")
        return search.best_estimator_, scaler
    except Exception as e:
        print(f"Hyperparameter tuning failed: {e}")
        # Fallback to default model
        default_model = SymbolicRegressor(
            population_size=500,
            generations=15,
            function_set=('add', 'sub', 'mul', 'div'),
            parsimony_coefficient=0.01,
        ...
        print(f"Fallback symbolic expression: {default_model._program}")
        return default_model, scaler
...
model, scaler = train_symbolic_regressor(data, n_iter_search=8)
...
while iteration < max_iterations and not found_3_selmer:
    ...
    # Predict best pairs
    top_pairs = predict_best_pairs(model, scaler, fib_list, num_candidates=3)
    ...
    if not found_3_selmer:
        ...
        model, scaler = train_symbolic_regressor(data, n_iter_search=5)
        iteration += 1

# Final result
if found_3_selmer:
    print(f"\nFound a curve with computable 3-Selmer rank after {iteration + 1} iterations.")
    print(f"Final symbolic expression: {model._program}")
else:
