    ...
    try:
        ...
        try:
            ...
            if rank != descent_rank:
                ...
                rank = descent_rank
            # Point search for original curve
            if is_original:
                points = E.points(bound=10)
                non_torsion = [p for p in points if p.order() == 0]
                if non_torsion:
                    print(f"Found non-torsion points: {non_torsion}")
                else:
                    print("No non-torsion points found in search")
        except:
        ...
        try:
            ...
            try:
                # Fallback: Estimate via 3-descent
                E.three_descent()
                print("3-Selmer rank approximated via descent")
            except:
