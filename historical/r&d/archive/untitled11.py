...
def random_fibonacci_pair(n):
    """Select a random pair of Fibonacci numbers up to index n."""
    fib_list = generate_fibonacci(n)
    # Exclude pairs that might cause issues (e.g., (0, 0))
...
def analyze_curve(a, b, is_original=False, max_attempts=5):
    """Analyze an elliptic curve, retrying rank calculations if they fail."""
    ...
    # Initialize flags for successful calculations
    rank_success = False
    ...
    # Attempt rank and Selmer calculations
    for attempt in range(max_attempts):
        try:
            ...
            break  # Exit loop if rank and 2-Selmer succeed
        except:
    ...
    if not (rank_success and selmer2_success and selmer3_success):
        print("Failed to compute all required ranks")
        return False

    # Compute L-function and BSD invariants
    try:
        L = E.lseries()
...
max_successful_curves = 10  # Number of curves to successfully analyze
max_total_attempts = 100  # Prevent infinite loops
n = 30  # Max Fibonacci index for diversity
successful_curves = 0
attempts = 0
