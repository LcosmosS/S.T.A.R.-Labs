...
def random_fibonacci_pair(n, classifier=None, fib_list=None, X_data=None, force_failure=False, seen_pairs=None):
    ...
    high_rank_pairs = [(2, 144), (3, 144), (5, 144), (2, 233), (8, 610), (5, 377), (34, 610)]
    high_rank_fibs = [2, 144, 3, 5, 233, 8, 610, 377, 34]
    seen_pairs = seen_pairs or {}

    if classifier is None or X_data is None or len(X_data) < 10:
        if random.random() < 0.995 and high_rank_pairs:  # 99.5% bias
...
def analyze_curve(a, b, is_original=False, max_attempts=5, require_3selmer=False):
            ...
            try:
                ...
                if selmer3_rank >= 3:
                    ...
                    with open("rank3_curves.txt", "a") as f:
                        vol = omega * reg * (VIRGO_DISTANCE / (omega * SQRT_KAPPA))**3 / 3e13 if omega and reg else 'N/A'
    ...
    if success:
        try:
            ...
            comoving_volume = (omega * reg * cosmo_scale**3) / (3e13 if rank == 3 else 1e14)
            ...
            print(f"Scaled regulator (Reg * √κ * {'20' if rank == 3 else '10' if rank == 2 else '20' if rank == 1 else '20'}): {float(scaled_reg)} (density height)")
            print(f"Product of Tamagawa numbers: {tamagawa} (local edge constraints)")
            print(f"Estimated comoving volume (Omega * Reg * scale^3 / {'3e13' if rank == 3 else '1e14'}): {comoving_volume} Mly^3")
    ...
    if success and rank is not None:
        try:
            x_range = 3 * math.sqrt(abs(a)) if abs(a) > 1 else 10
            x_vals = np.linspace(-x_range, x_range, 1000)
            y_vals = np.sqrt(np.maximum(x_vals**3 + a * x_vals + b, 0))
            density_size = min(leading_coeff * 177 / 100, 70) if leading_coeff else 10  # Cap size
            ...
            if rank > 0:
                contour_y = np.full_like(x_vals, float(scaled_reg / 2000))
