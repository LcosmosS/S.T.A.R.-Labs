...
def analyze_curve(a, b, is_original=False):
    ...
    try:
        rank = E.rank()
        # Verify with 2-descent
        E.two_descent(verbose=False)
        selmer_rank = E.selmer_rank()
        two_torsion_rank = 1 if tors_order % 2 == 0 else 0
        if selmer_rank - two_torsion_rank < rank:
            print(f"Warning: Rank {rank} may be incorrect, descent suggests rank <= {selmer_rank - two_torsion_rank}")
    ...
    D = -3  # Quadratic imaginary field Q(sqrt(-3))
    try:
