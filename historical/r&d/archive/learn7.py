...
while successful_curves < max_successful_curves and attempts < max_total_attempts:
    ...
    success, features, rank, leading_coeff, omega, reg, tamagawa = analyze_curve(a, b, require_3selmer=require_3selmer)
    if features:
        X_data.append(features)
        y_data.append(success)
        if success and rank is not None:
            interweb_data.append((a, b, rank, leading_coeff, omega, reg, tamagawa, features[2], features[3]))
    if success and rank is not None:
        successful_curves += 1
    attempts += 1
...
success, features, rank, leading_coeff, omega, reg, tamagawa = analyze_curve(-1706, 6320, is_original=True, require_3selmer=require_3selmer)
if features:
    X_data.append(features)
    y_data.append(success)
    if success and rank is not None:
        interweb_data.append((-1706, 6320, rank, leading_coeff, omega, reg, tamagawa, features[2], features[3]))
